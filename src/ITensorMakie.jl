...
import ITensorVisualizationBase: _graphplot, visualize, visualize!
using GraphMakie.Makie: Figure, Makie, contents, deregister_interaction!, hidedecorations!,
    hidespines!, register_interaction!
using ITensorVisualizationBase: @Backend_str, _ndims, default_arrow_show,
    default_arrow_size, default_edge_labels, default_edge_textsize, default_edge_widths,
...
function visualize!(
    ...
    graphplot_kwargs = (;
        ...
        # self-edge
        selfedge_width = 1.0e-5, # Small enough so you can't see the loop, big enough for site label to show up
    ...
    if !overwrite_axis && (_ndims(layout) == 2)
        ...
        if interactive
            ...
            register_interaction!(
                axis_plot.axis,
                :nhover,
                NodeHoverHighlight(axis_plot.plot)
            )
            register_interaction!(
                axis_plot.axis,
                :ehover,
                EdgeHoverHighlight(axis_plot.plot)
            )
